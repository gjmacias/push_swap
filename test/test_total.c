...
void	finish_him(t_parameters *p)
{
	...
	is_min = 0;
	tmp = ft_last(&p->a);
	if (p->b->number < ft_max(&p->a))
	{
	    while (!(p->b->number > tmp) && !(ft_min(&p->a) > p->b->number))
	    {
	        rr_stack(&p->a, 'a');
			tmp = ft_last(&p->a);
	    }
	...
	{
		if (tmp > p->b->number && !(ft_min(&p->a) == p->a->number))
		{
			rr_stack(&p->a, 'a');
			tmp = ft_last(&p->a);
		}
	...
	if (p->b->number > ft_max(&p->a) || p->b->number < ft_min(&p->a))
	{
	    while ((tmp != ft_max(&p->a)) && is_min++ != 4) 
	        rr_stack(&p->a, 'a');
	}
	else 
	{
	    while (!(p->b->number > tmp && p->b->number < p->a->number) && is_min++ != 4) 
	        rr_stack(&p->a, 'a');
	}
    tmp = ft_last(&p->a);
	while (check_length(&p->a) != p->length)
